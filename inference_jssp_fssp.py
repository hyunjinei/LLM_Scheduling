...
HF_DEFAULT_TOKEN = os.environ.get("HF_TOKEN", "")
import csv
...
parser = argparse.ArgumentParser(description="Inference with FastLanguageModel with specified parameters.")
...
parser.add_argument('--max_new_tokens', type=int, default=40000, help='Maximum new tokens to generate')  # 5000 -> 40000
...
parser.add_argument('--model_path', type=str, default=None, 
                    help='Path to the finetuned model (if None, will be inferred from model_type)')
...
parser.add_argument('--output_accord', action='store_true', default=False, 
                    help='Whether to use ACCORD-style output or not')
parser.add_argument('--output_list_of_lists', action='store_true', default=False, 
                    
                    help='Whether to use list of lists as output or not')

# Task options
parser.add_argument('--use_distance', action='store_true', default=True, 
                    help='Whether to use paired distance info in the input or not')
parser.add_argument('--infer_fssp', action='store_true', default=False,  
                    help='Infer flowshop scheduling problem or not')

# Task type options (unified_trainer.py와 일관성)
...
parser.add_argument('--disable_masking', action='store_true', default=False, help='Disable decoding-time masking constraints')
parser.add_argument('--hf_token', type=str, default=None, help='Hugging Face access token (overrides default)')
parser.add_argument('--demo_index', type=int, default=None, help='Run a single validation instance by index and print results instead of full evaluation')
parser.add_argument('--demo_num_solutions', type=int, default=None, help='Override number of solutions when running --demo_index')
parser.add_argument('--inspect_masking', action='store_true', default=False, help='Print detailed masking states during demo runs')
parser.add_argument('--step_action_max_new_tokens', type=int, default=1, help='Max new tokens for one step action decode')
parser.add_argument('--disable_step_problem_context', action='store_true', default=False, help='Do not include static problem context in each step prompt')
parser.add_argument('--enable_step_improvement', action='store_true', default=False, help='Enable post-episode critical-step improvement passes')
parser.add_argument('--step_reflection_passes', type=int, default=1, help='Number of post-episode improvement passes when --enable_step_improvement is on')
parser.add_argument('--step_reflection_topk', type=int, default=3, help='Number of critical steps to guide per improvement pass')
parser.add_argument('--emit_step_rationale', action='store_true', default=False, help='Generate explanation text after each step action (does not affect action)')
parser.add_argument('--step_rationale_max_new_tokens', type=int, default=96, help='Max new tokens for per-step rationale generation')
parser.add_argument('--reason_model_path', type=str, default=None, help='Optional separate reason adapter path/repo. If set, rationale generation uses adapter switching.')
parser.add_argument('--reason_topk', type=int, default=3, help='Number of strongest alternatives to expose in reason input prompt.')
parser.add_argument('--eval_data_path', type=str, default=None, help='Optional JSON file path for JSSP/FSSP evaluation dataset')
...
def _resolve_model_path(raw_model_path: str | None) -> str:
    model_path = raw_model_path or "HYUNJINI/jssp_llama8b_accord_r64_ep4"
...
def main():
    ...
    base_model_name = _resolve_base_model_name(run_args.model_type)
    run_args.model_path = _resolve_model_path(run_args.model_path)
    run_args.reason_model_path = (
        _resolve_model_path(run_args.reason_model_path)
        if run_args.reason_model_path
        else None
    )

    token_candidates = [run_args.hf_token, HF_DEFAULT_TOKEN]
    hf_token = next((tok for tok in token_candidates if tok), None)
    if hf_token:
        print("🔐 Hugging Face Hub 로그인 시도 중...")
        login(token=hf_token, add_to_git_credential=False)
    else:
        print("ℹ️ Hugging Face 토큰이 제공되지 않았습니다. 공개 모델인 경우에만 다운로드가 가능합니다.")

    print(f"Using model: {base_model_name}")
    print(f"Model path: {run_args.model_path}")
    if run_args.reason_model_path and run_args.emit_step_rationale:
        print(f"Reason adapter path: {run_args.reason_model_path}")
    print(f"Loading model from: {run_args.model_path}")
    print(f"Max sequence length: {run_args.max_seq_length}")
    print(f"Data type: {run_args.dtype}")
    print(f"Load in 4bit: {run_args.load_in_4bit}")

    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name=run_args.model_path,
        max_seq_length=run_args.max_seq_length,
        load_in_4bit=run_args.load_in_4bit,
        dtype=torch.bfloat16 if run_args.dtype == "bfloat16" else torch.float16,
        local_files_only=False,
    )
    token_install = ensure_action_special_tokens(
        tokenizer=tokenizer,
        model=model,
        code_width=run_args.action_code_width,
        code_cap=run_args.action_code_cap,
    )
    print(
        "Action token install:",
        token_install,
    )
    ...
    if run_args.reason_model_path and run_args.emit_step_rationale:
        _maybe_load_reason_adapter(model, run_args.reason_model_path)
        print("Reason adapter loaded successfully.")

    eval_dataset = _load_eval_dataset(run_args)

    print("Starting evaluation loop...")
    task_type = "fssp" if run_args.infer_fssp else "jssp"
    output_style = "accord" if run_args.output_accord else "list_of_lists"
    output_dir = run_args.output_dir if run_args.output_dir else f"val_results/{task_type}_val"
    os.makedirs(output_dir, exist_ok=True)

    model_base_name = os.path.basename(run_args.model_path) if run_args.model_path else "model"
    csv_filename = f"{output_dir}/output_step_{output_style}_{model_base_name}_num_return_sequences{run_args.num_return_sequences}.csv"
    aggregate_csv_filename = f"{output_dir}/evaluate_all_problems_{model_base_name}_num_return_sequences{run_args.num_return_sequences}.csv"

    print("🚀 step-by-step 추론 시작 (one-shot 제거됨)")
    print(f"🔒 Decoding-time masking enabled: {not run_args.disable_masking}")
    print(f"📁 결과 저장 경로: {csv_filename}")
    print(f"📁 집계 저장 경로: {aggregate_csv_filename}")

    if run_args.demo_index is not None:
        demo_num_solutions = run_args.demo_num_solutions if run_args.demo_num_solutions is not None else run_args.num_return_sequences
        run_demo_instance_step(
            model=model,
            tokenizer=tokenizer,
            dataset=eval_dataset,
            run_args=run_args,
            index=run_args.demo_index,
            num_solutions=demo_num_solutions,
            use_masking=not run_args.disable_masking,
        )
        return
    ...
    evaluate_model_step(
        ...
        aggregate_csv_filename=aggregate_csv_filename,
        use_masking=not run_args.disable_masking,
    )
    print("✅ 추론 완료! solution_generation_english 기반 고급 검증 완료")
    print("📊 결과 저장 위치:", csv_filename)
    print("📊 집계 저장 위치:", aggregate_csv_filename)
